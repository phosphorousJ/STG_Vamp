using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.SceneManagement;

public class E_TT_SkillAttack1Manager : MonoBehaviour
{
    #region//インスペクター設定
    //Playerオブジェクトを入れる
    public GameObject Player;

    //E_TT_SkillAttack1_2Prefabを入れる（刺突）
    public GameObject E_TT_SkillAttack1_2Prefab;

    //E_TT_SkillAttack1_3Prefabを入れる（刺突斬り）
    public GameObject E_TT_SkillAttack1_3Prefab;
    #endregion


    #region//プライベート設定
    //Playerの座標
    private Vector3 player;
    #endregion


    // Start is called before the first frame update
    void Start()
    {
        //TTの大技1発動を判定
        GManager.instance.TT_Skill1 = true;

        //コルーチン開始
        StartCoroutine(SkillAttack1_0());
    }


    #region//0:刺突E、刺突E、刺突斬りE、刺突S、刺突S、刺突斬りN
    IEnumerator SkillAttack1_0()
    {
        var waitOneS = new WaitForSeconds(1.0f);
        var waitOneHalfS = new WaitForSeconds(1.5f);


        yield return new WaitForSeconds(2.0f);


        //刺突E
        player = Player.transform.position;
        E_TT_SkillAttack1_2_X(4.3f, player.y, 90);
        

        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突E
        player = Player.transform.position;
        E_TT_SkillAttack1_2_X(4.3f, player.y, 90);
       

        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突斬りE
        player = Player.transform.position;
        E_TT_SkillAttack1_3_X(3.5f, player.y, 90);


        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突S
        player = Player.transform.position;
        E_TT_SkillAttack1_2_Y(player.x, -4.3f, 0);

        
        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突S
        player = Player.transform.position;
        E_TT_SkillAttack1_2_Y(player.x, -4.3f, 0);


        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突斬りN
        player = Player.transform.position;
        E_TT_SkillAttack1_3_Y(player.x, 3.5f, 180);

        
        yield return waitOneS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();

        StartCoroutine(SkillAttack1_1());
    }
    #endregion

    #region//1:刺突斬りS、刺突斬りW、刺突斬りN、刺突斬りE、刺突N、刺突S
    IEnumerator SkillAttack1_1()
    {
        var waitOneS = new WaitForSeconds(1.0f);
        var waitOneHalfS = new WaitForSeconds(1.5f);


        yield return waitOneS;


        //刺突斬りS
        player = Player.transform.position;
        E_TT_SkillAttack1_3_Y(player.x, -3.5f, 0);


        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突斬りW
        player = Player.transform.position;
        E_TT_SkillAttack1_3_X(-3.5f, player.y, -90);
        

        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突斬りN
        player = Player.transform.position;
        E_TT_SkillAttack1_3_Y(player.x, 3.5f, 180);


        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突斬りE
        player = Player.transform.position;
        E_TT_SkillAttack1_3_X(3.5f, player.y, 90);


        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突N
        player = Player.transform.position;
        E_TT_SkillAttack1_2_Y(player.x, 4.3f, 180);


        yield return waitOneHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突S
        player = Player.transform.position;
        E_TT_SkillAttack1_2_Y(player.x, -4.3f, 0);


        yield return waitOneS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();
        
        StartCoroutine(SkillAttack1_2());
    }
    #endregion

    #region//2:刺突N、刺突S、刺突N、刺突S、刺突N、刺突S、刺突斬りN
    IEnumerator SkillAttack1_2()
    {
        var waitHalfS = new WaitForSeconds(0.5f);
        var waitOneS = new WaitForSeconds(1.0f);


        yield return waitOneS;


        for (int i = 0; i < 3; i++)
        {
            //刺突N
            player = Player.transform.position;
            E_TT_SkillAttack1_2_Y(player.x, 4.3f, 180);


            yield return waitHalfS;

            //生成位置をリセット
            E_TT_SkillAttack1Reset();


            //刺突S
            player = Player.transform.position;
            E_TT_SkillAttack1_2_Y(player.x, -4.3f, 0);


            yield return waitHalfS;

            //生成位置をリセット
            E_TT_SkillAttack1Reset();
        }


        //刺突斬りN
        player = Player.transform.position;
        E_TT_SkillAttack1_3_Y(player.x, 3.5f, 180);


        yield return waitOneS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();

        StartCoroutine(SkillAttack1_3());
    }
    #endregion

    #region//3:刺突E、刺突W、刺突E、刺突W、刺突斬りS、刺突斬りN、刺突S、参命剣N
    IEnumerator SkillAttack1_3()
    {
        var waitHalfS = new WaitForSeconds(0.5f);
        var waitOneS = new WaitForSeconds(1.0f);


        yield return waitOneS;


        for (int i = 0; i < 2; i++)
        {
            //刺突E
            player = Player.transform.position;
            E_TT_SkillAttack1_2_X(4.3f, player.y, 90);


            yield return waitHalfS;

            //生成位置をリセット
            E_TT_SkillAttack1Reset();


            //刺突W
            player = Player.transform.position;
            E_TT_SkillAttack1_2_X(-4.3f, player.y, -90);


            yield return waitHalfS;

            //生成位置をリセット
            E_TT_SkillAttack1Reset();
        }


        //刺突斬りS
        player = Player.transform.position;
        E_TT_SkillAttack1_3_Y(player.x, -3.5f, 0);


        yield return waitOneS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突斬りN
        player = Player.transform.position;
        E_TT_SkillAttack1_3_Y(player.x, 3.5f, 180);


        yield return waitOneS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //刺突S
        player = Player.transform.position;
        E_TT_SkillAttack1_2_Y(player.x, -4.3f, 0);

        
        yield return waitHalfS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();


        //参命剣N
        player = Player.transform.position;
        E_TT_SkillAttack1_2_Y(player.x - 0.66f, 4.3f, 180);
        E_TT_SkillAttack1_2_Y(player.x, 4.3f, 180);
        E_TT_SkillAttack1_2_Y(player.x + 0.66f, 4.3f, 180);


        yield return waitOneS;

        //生成位置をリセット
        E_TT_SkillAttack1Reset();

        SceneManager.LoadScene("TalkScene1_4");
    }
    #endregion


    #region//攻撃の処理関数
    //刺突（N&S）の処理関数
    void E_TT_SkillAttack1_2_Y(float positionX, float positionY, float angle)
    {
        //刺突（E_TT_SkillAttack1_2Prefab）を生成
        Instantiate(E_TT_SkillAttack1_2Prefab, new Vector3(positionX, positionY, -0.05f), Quaternion.Euler(0, 0, angle));

        //生成位置を保存
        GSubManager.instance.TT_SkillAttack0_2PosY = positionY;
    }

    //刺突（W&E）の処理関数
    void E_TT_SkillAttack1_2_X(float positionX, float positionY, float angle)
    {
        //刺突（E_TT_SkillAttack1_2Prefab）を生成
        Instantiate(E_TT_SkillAttack1_2Prefab, new Vector3(positionX, positionY, -0.05f), Quaternion.Euler(0, 0, angle));

        //生成位置を保存
        GSubManager.instance.TT_SkillAttack0_2PosX = positionX;
    }


    //刺突斬り（N&S）の処理関数
    void E_TT_SkillAttack1_3_Y(float positionX, float positionY, float angle)
    {
        //刺突斬り（E_TT_SkillAttack1_3Prefab）を生成
        Instantiate(E_TT_SkillAttack1_3Prefab, new Vector3(positionX, positionY, -0.05f), Quaternion.Euler(0, 0, angle));

        //生成位置を保存
        GSubManager.instance.TT_SkillAttack0_3PosY = positionY;
    }

    //刺突斬り（W&E）の処理関数
    void E_TT_SkillAttack1_3_X(float positionX, float positionY, float angle)
    {
        //刺突斬り（E_TT_SkillAttack1_3Prefab）を生成
        Instantiate(E_TT_SkillAttack1_3Prefab, new Vector3(positionX, positionY, -0.05f), Quaternion.Euler(0, 0, angle));

        //生成位置を保存
        GSubManager.instance.TT_SkillAttack0_3PosX = positionX;
    }
    #endregion


    #region//生成位置のリセット関数
    //攻撃の生成位置をリセットする関数
    void E_TT_SkillAttack1Reset()
    {
        GSubManager.instance.TT_SkillAttack0_2PosX = 0.0f;
        GSubManager.instance.TT_SkillAttack0_2PosY = 0.0f;
        GSubManager.instance.TT_SkillAttack0_3PosX = 0.0f;
        GSubManager.instance.TT_SkillAttack0_3PosY = 0.0f;
    }
    #endregion
}
