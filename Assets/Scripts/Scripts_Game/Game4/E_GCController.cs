...
public class E_GCController : MonoBehaviour
{
    #region//インスペクター設定
    //E_NomalAttack0Prefab（通常攻撃）を入れる
    public GameObject E_NomalAttack0Prefab;

    //E_NomalAttack1Prefab（通常攻撃）を入れる
    public GameObject E_NomalAttack1Prefab;
    ...
    void GenAttackKind()
    {
        ...
        //攻撃の部分割合（30%:通常0,30%:通常1,20%:青,15%:緑,5%:赤）
        if (1 <= attack && attack <= 3)
        {
            GameObject nomal = Instantiate(E_NomalAttack0Prefab);
            nomal.transform.position = new Vector3(laneX, nStartPosY, nStartPosZ);
        }
        else if (3 <= attack && attack <= 6)
        {
            GameObject nomal = Instantiate(E_NomalAttack1Prefab);
            nomal.transform.position = new Vector3(laneX, nStartPosY, nStartPosZ);
        }
